...
if [[ -z "${TURN_BUILD_SERVER_URL:-}" ]]; then
    _builder_client_libdir="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
    source "$_builder_client_libdir/../build-server/server-port.sh"
    _builder_client_root="$(cd "$_builder_client_libdir/../.." && pwd)"
    _builder_client_port_file="$_builder_client_root/.watch/builder.port"
    if [[ -s "$_builder_client_port_file" ]]; then
        TURN_BUILD_SERVER_URL="http://127.0.0.1:$(cat "$_builder_client_port_file")"
    else
        TURN_BUILD_SERVER_URL="http://127.0.0.1:$(_builder_port_for "$_builder_client_root")"
    fi
    unset _builder_client_libdir _builder_client_root _builder_client_port_file
fi
...
_srv_forward() {
    ...
    curl -sS -N -X POST "$TURN_BUILD_SERVER_URL/run" -H 'Content-Type: application/json' --data "$payload" 2>/dev/null \
        | python3 "$libdir/../build-server/server-filter.py"
